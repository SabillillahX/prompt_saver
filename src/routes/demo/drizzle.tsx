...
const createPrompt = createServerFn({
  method: 'POST',
})
  .inputValidator((data: { title: string }) => data)
  .handler(async ({ data }) => {
    await db.insert(promptsTable).values({ title: data.title, content: '' })
    return { success: true }
  })

export const Route = createFileRoute('/demo/drizzle')({
  component: DemoDrizzle,
  loader: async () => await getPrompts(),
})

function DemoDrizzle() {
  const router = useRouter()
  const promptsData = Route.useLoaderData()
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="text-2xl font-bold mb-4 text-indigo-200">Prompts</h2>
          ...
          {promptsData.map((prompt: any) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-indigo-300/70">#{prompt.id}</span>
              </div>
            </li>
          ))}
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            ...
            style={{
              ...
              borderColor: 'rgba(93, 103, 227, 0.3)',
              // Remove focusRing, not a CSS property
            }}
          />
          <button
            type="submit"
